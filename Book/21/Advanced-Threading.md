...
علاوه بر `Set`, `WaitOne`, و `Reset`، کلاس `WaitHandle` متدهای استاتیک زیر را برای **همگام‌سازی پیچیده‌تر** فراهم می‌کند:

1. **WaitAny**: منتظر **هر یک** از wait handleهای داده‌شده می‌ماند.
2. **WaitAll**: منتظر **تمام** wait handleها می‌ماند، به صورت اتمیک.

   * مثال: اگر روی دو AutoResetEvent منتظر باشید:

     * WaitAny هیچ‌گاه هر دو event را همزمان latch نمی‌کند.
     * WaitAll هیچ‌گاه فقط یکی از eventها را latch نمی‌کند.
3. **SignalAndWait**: ابتدا `Set` روی یک WaitHandle و سپس `WaitOne` روی دیگری انجام می‌دهد.

   * کمک می‌کند دو نخ در یک نقطه rendezvous شوند.
   * مثال:

     * نخ اول: `WaitHandle.SignalAndWait(wh1, wh2);`
     * نخ دوم: `WaitHandle.SignalAndWait(wh2, wh1);`

* این متدها برای **multi-threaded apartment** نیاز دارند و در STA (single-threaded apartment) کار نمی‌کنند.
* اگر atomicity نیاز ندارید، می‌توانید wait handleها را به **Task** تبدیل کرده و از `Task.WhenAny` و `Task.WhenAll` استفاده کنید.
* اگر atomicity نیاز دارید، می‌توانید از روش سطح پایین با `Monitor.Wait` و `Monitor.Pulse` استفاده کنید.
