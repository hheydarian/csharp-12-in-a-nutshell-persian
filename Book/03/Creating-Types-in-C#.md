...
جعبه‌گذاری کنش تبدیل یک نمونه‌ی گونه‌ی ارزشی به یک نمونه‌ی گونه‌ی ارجاعی است. گونه‌ی ارجاعی 
می‌تواند هم کلاس `object` یا یک میانجی باشد (که در آینده در این پاره به آن می‌پردازیم). در 
این نمونه، ما یک `int` را به درون یک `object` جعبه‌گذاری می‌کنیم:
```Cs

int x = 9;
object obj = x; // int را جعبه‌گذاری کنید


```
**جعبه‌گشایی** با نوع‌ریزی شیء به گونه‌ی ارزشی اصلی، عملیات را برعکس می‌کند:
```Cs
int y = (int)obj; // int را جعبه‌گشایی کنید
```
جعبه‌گشایی نیاز به یک نوع‌ریزی آشکار دارد. زمان اجرا بررسی می‌کند که گونه‌ی ارزشی بیان‌شده 
با گونه‌ی واقعی شیء همسان باشد و اگر بررسی شکست بخورد، یک `InvalidCastException` پرتاب 
می‌کند. برای نمونه، نمونه‌ی زیر یک کژکاری پرتاب می‌کند، چرا که `long` با `int` به گونه‌ی 
دقیق همسان نیست:
```Cs
...
```
در نمونه‌ی پایانی، (`double`) یک جعبه‌گشایی انجام می‌دهد و سپس (`int`) یک تبدیل عددی را 
انجام می‌دهد.

+ تبدیل‌های جعبه‌گذاری در فراهم کردن یک سامانه‌ی گونه‌ی یگانه حیاتی هستند. با این همه، این 
+ سامانه بی‌نقص نیست: در بخش «عمومی‌ها (Generics)» در رویه‌ی 159 خواهیم دید که دگرگونی‌پذیری 
+ (variance) با آرایه‌ها و عمومی‌ها تنها از تبدیل‌های ارجاعی پشتیبانی می‌کند و نه از تبدیل‌های  
+ جعبه‌گذاری:
```Cs
...
### معناشناسی کپی‌برداری در جعبه‌گذاری و جعبه‌گشایی
جعبه‌گذاری، نمونه‌ی گونه‌ی ارزشی را به درون شیء نو کپی می‌کند و جعبه‌گشایی، محتویات شیء را 
دوباره به درون یک نمونه‌ی گونه‌ی ارزشی کپی می‌کند. در نمونه‌ی زیر، دگرگون کردن ارزش `i`، کپی 
پیشین جعبه‌گذاری‌شده‌ی آن را دگرگون نمی‌کند:
